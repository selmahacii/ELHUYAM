                  ...
                  <div key={item.id} className="flex gap-3 p-4 hover:bg-zinc-50/20 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-zinc-400 mt-1.5 font-mono">
                        {item.quantity} x {formatPrice(item.price, order.isInternational ? "EUR" : "DZD")}
                      </p>
                    </div>
          ...
          <div className="bg-white border border-zinc-200/80 shadow-sm rounded-sm p-4 space-y-3">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between text-zinc-500">
                <span>Subtotal</span>
                <span className="font-mono">{formatPrice(order.subtotal, order.isInternational ? "EUR" : "DZD")}</span>
              </div>
              <div className="flex justify-between text-zinc-500">
                <span>Shipping fee</span>
                <span className="font-mono">{order.shippingFee === 0 ? "Free" : formatPrice(order.shippingFee, order.isInternational ? "EUR" : "DZD")}</span>
              </div>
              {order.discount > 0 && (
                <div className="flex justify-between text-emerald-600 font-medium">
                  <span>Discount {order.couponCode && `(${order.couponCode})`}</span>
                  <span className="font-mono">-{formatPrice(order.discount, order.isInternational ? "EUR" : "DZD")}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-between font-bold text-zinc-900 text-xs border-t border-zinc-150 pt-2.5">
              ...
              <span className="text-sm text-zinc-950 font-mono">{formatPrice(order.totalAmount, order.isInternational ? "EUR" : "DZD")}</span>
            </div>
