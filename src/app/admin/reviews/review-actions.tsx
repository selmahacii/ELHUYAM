      ...
      <div className="flex items-center gap-1.5 shrink-0 self-center">
        ...
        {status !== "APPROVED" && (
          <button
            ...
            className="p-2 text-emerald-600 hover:bg-emerald-50 border border-transparent hover:border-emerald-100 rounded-xl transition-all shadow-2xs disabled:opacity-50 flex items-center justify-center h-9 w-9"
          >
            {loading === "APPROVED" ? (
        ...
        {status !== "REJECTED" && (
          <button
            ...
            disabled={isWorking}
            title={t("rejectTitle")}
            className="p-2 text-amber-600 hover:bg-amber-50 border border-transparent hover:border-amber-100 rounded-xl transition-all shadow-2xs disabled:opacity-50 flex items-center justify-center h-9 w-9"
          >
            {loading === "REJECTED" ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
        ...
        <button
          ...
          disabled={isWorking}
          title={t("deleteTitle")}
          className="p-2 text-red-500 hover:bg-red-50 border border-transparent hover:border-red-100 rounded-xl transition-all shadow-2xs disabled:opacity-50 flex items-center justify-center h-9 w-9"
        >
          {loading === "DELETE" ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
